function [Va_Upwind] = vp_upwind_q3(v0,param,num,grid,M)
...
Vab(1) = (param.A*grid.a(1)^param.alpha - param.r*grid.a(1)).^(-1); %state constraint boundary condition    


%consumption and savings with forward difference
cf = max(Vaf,1e-08).^(-1);
Va_Upwind.sf = M + param.r*grid.a - cf ;
%consumption and savings with backward difference
cb = max(Vab,1e-08).^(-1);
Va_Upwind.sb = M + param.r*grid.a - cb ;
